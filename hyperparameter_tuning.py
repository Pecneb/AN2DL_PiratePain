...
def scale_dataset(df: pd.DataFrame, method: str = "extra") -> pd.DataFrame:
    """
    ...
    feature_cols = [col for col in _df.columns 
    ...
    if method == "inter":
        for sample_idx in _df["sample_index"].unique():
            ...
            _df.loc[mask, feature_cols] = scaler.fit_transform(_df.loc[mask, feature_cols])
...
def build_sequences(df: pd.DataFrame, labels_df: pd.DataFrame) -> Tuple[np.ndarray, np.ndarray]:
    """Build sequences from dataframe, dropping metadata columns."""
    dataset = []
    labels = []
    
    for idx in df["sample_index"].unique():
        person_ds = df[df["sample_index"] == idx].drop(
            columns=["sample_index", "time"]
        ).to_numpy()
...
class RecurrentClassifierPL(pl.LightningModule):
    ...
    def on_train_epoch_end(self):
        ...
        if len(self.training_step_outputs) > 0:
            all_preds = torch.cat([x['preds'] for x in self.training_step_outputs]).cpu().numpy()
            all_targets = torch.cat([x['targets'] for x in self.training_step_outputs]).cpu().numpy()
...
def run_kfold_cv(
    ...
    if verbose:
        ...
        print(f"{'='*60}")
        print(f"Mean Val F1:  {results['mean_val_f1']:.4f} ± {results['std_val_f1']:.4f}")
...
def optuna_optimization(
    ...
    def objective(trial: optuna.Trial) -> float:
        ...
        config = {
            ...
            'criterion_name': trial.suggest_categorical('criterion_name', ['CrossEntropy', 'FocalLoss', 'LabelSmoothing']),
        }
...
def main():
    parser = argparse.ArgumentParser(description='Hyperparameter Tuning for Pirate Pain Classification')
    parser.add_argument('--data_dir', type=str, default='./data', help='Data directory')
    parser.add_argument('--method', type=str, default='grid', choices=['grid', 'optuna'], help='Search method')
    ...
    parser.add_argument('--scaling', type=str, default='extra', choices=['inter', 'extra', 'hybrid'], help='Scaling method')
    parser.add_argument('--save_dir', type=str, default='./lightning_logs_tuning', help='Save directory')
    parser.add_argument('--results_file', type=str, default='./hyperparameter_search_results.csv', help='Results CSV file')
    parser.add_argument('--n_trials', type=int, default=50, help='Number of Optuna trials (only for --method optuna)')
    ...
    df = scale_dataset(df, method=args.scaling)
