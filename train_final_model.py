...
def load_and_preprocess_data(
    ...
    # Metad columns
    meta_cols = ["sample_idx", "time", "label"]

    # Features to use
    joint_cols = [col for col in train_df.columns if col.startswith("joint_")]
    # bodypart_cols = ['n_legs', 'n_eyes', 'n_hands']
    bodypart_cols = ["is_pirate"]
    pain_surveys = [col for col in train_df.columns if col.startswith("pain_survey_")]
    feature_cols = joint_cols + bodypart_cols + pain_surveys

    print(
        f"✓ Using {len(feature_cols)} features ({len(joint_cols)} joints + {len(bodypart_cols)} body parts)"
    )
...
def train_final_model(
    ...
    print(
        f"✓ Created model: {config['rnn_type']} (hidden_size={config['hidden_size']}, num_layers={config['num_layers']})"
    )
    ...
    # Setup logger
    logger = TensorBoardLogger(save_dir=output_dir, name="final_model_logs")
